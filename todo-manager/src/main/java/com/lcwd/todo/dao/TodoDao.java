...
public class TodoDao {
    ...
    public TodoDao(@Autowired JdbcTemplate template) {
        ...
        String createTable="create table IF NOT EXISTS todos(id int primary key," +
                " title varchar(100) not null,content varchar(500)," +
                " status varchar(10) not null, addedDate datetime, todoDate datetime )";
...
//        return todos;
    ...
    public Todo updateTodo(int id, Todo newTodo){
        ...
        int update = template.update(query, newTodo.getTitle(), newTodo.getContent(), newTodo.getStatus(), newTodo.getToDoDate(), newTodo.getAddedDate(), id);
